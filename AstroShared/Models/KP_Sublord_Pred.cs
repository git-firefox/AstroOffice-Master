using System;
using System.Runtime.CompilerServices;

namespace AstroShared.Models
{
    public class KP_Sublord_Pred
    {
        public bool bachpan
        {
            get;
            set;
        }

        public bool brother
        {
            get;
            set;
        }

        public bool budhapa
        {
            get;
            set;
        }

        public bool children
        {
            get;
            set;
        }

        public bool common
        {
            get;
            set;
        }

        public bool disease
        {
            get;
            set;
        }

        public bool female
        {
            get;
            set;
        }

        public bool general
        {
            get;
            set;
        }

        public short House
        {
            get;
            set;
        }

        public bool Isbad
        {
            get;
            set;
        }

        public bool kishor
        {
            get;
            set;
        }

        public bool love_affair
        {
            get;
            set;
        }

        public bool madhya
        {
            get;
            set;
        }

        public bool male
        {
            get;
            set;
        }

        public bool married
        {
            get;
            set;
        }

        public bool mother_father
        {
            get;
            set;
        }

        public bool occupation
        {
            get;
            set;
        }

        public string pred_assamese
        {
            get;
            set;
        }

        public string pred_bengali
        {
            get;
            set;
        }

        public string Pred_English
        {
            get;
            set;
        }

        public string pred_gujarati
        {
            get;
            set;
        }

        public string Pred_Hindi
        {
            get;
            set;
        }

        public string pred_kannada
        {
            get;
            set;
        }

        public string pred_malayalam
        {
            get;
            set;
        }

        public string pred_marathi
        {
            get;
            set;
        }

        public string pred_oriya
        {
            get;
            set;
        }

        public string pred_punjabi
        {
            get;
            set;
        }

        public string pred_tamil
        {
            get;
            set;
        }

        public string pred_telugu
        {
            get;
            set;
        }

        public string PType
        {
            get;
            set;
        }

        public string RuleType
        {
            get;
            set;
        }

        public bool shishu
        {
            get;
            set;
        }

        public string ShowRef
        {
            get;
            set;
        }

        public long Sno
        {
            get;
            set;
        }

        public short Sublord
        {
            get;
            set;
        }

        public bool VeryBad
        {
            get;
            set;
        }

        public bool wealth
        {
            get;
            set;
        }

        public bool yuva
        {
            get;
            set;
        }

        public KP_Sublord_Pred()
        {
        }
    }
}