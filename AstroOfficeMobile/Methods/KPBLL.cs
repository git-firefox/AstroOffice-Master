    ...
    public class KPBLL
    {
        ...
        public string Get_Signi_String(short planet, List<KPPlanetMappingVO> kp_chart, bool include)
        {
            ...
            List<KPSigniVO> signi = (
                ...
                select Map).SingleOrDefault().Signi;
            short rule = 0;
        ...
        public List<KPDashaVO> Get_Dasha(List<KPHouseMappingVO> cusp_house, List<KPPlanetMappingVO> kp_chart, KundliVO persKV, bool include)
        {
            ...
            kPMahadashaVOs = Fill_Mahadasha();
            short nakNumber = 0;
            nak_list = Fill_Nak();
            short nakLord = (
                from Map in kp_chart
                where Map.Planet == 2
                select Map).SingleOrDefault().Nak_Lord;
            double degreePlanetDecimal = (
                from Map in kp_chart
                where Map.Planet == 2
                select Map).SingleOrDefault().DegreePlanet_Decimal;
            double rashi = (
                from Map in kp_chart
                where Map.Planet == 2
                select Map).SingleOrDefault().Rashi;
            KPDashaRashiVO kPDashaRashiVO = new KPDashaRashiVO();
            foreach (KPNAKVO nakList in nak_list)
            {
                ...
                kPDashaRashiVO = (
                    ...
                    where degreePlanetDecimal <= Map.Decimal_From || degreePlanetDecimal > Map.Decimal_To ? false : Map.Rashi == rashi
            ...
            for (short i = 1; i <= 9; i = (short)(i + 1))
            {
                kPDashaVO = new KPDashaVO()
                {
                    ...
                    Planet = (
                        from Map in kPMahadashaVOs
                        where Map.Sno == sno
                        select Map).SingleOrDefault().Planet,
                    StartDate = dateTime1,
                    EndDate = dateTime
                };
                TimeSpan endDate = kPDashaVO.EndDate - kPDashaVO.StartDate;
                kPDashaVO.Days = Convert.ToInt16(endDate.Days);
                ...
                kPDashaVO.Signi = Get_Planet_Signis(kp_chart, kPDashaVO.Planet, include);
                kPDashaVO.NakSigni = Get_Planet_Signis(kp_chart, (
                    from Map in kp_chart
                    where Map.Planet == kPDashaVO.Planet
                    select Map).SingleOrDefault().Nak_Lord, include);
                kPDashaVO.Signi_String = Get_Signi_String(kPDashaVO.Planet, kp_chart, include);
                kPDashaVO.Nak_Signi_String = Get_Signi_String((
                    from Map in kp_chart
                    where Map.Planet == kPDashaVO.Planet
                    select Map).SingleOrDefault().Nak_Lord, kp_chart, include);
            ...
            foreach (KPPlanetMappingVO kpChart in kp_chart)
            {
                if (!include)
                {
                    foreach (KPSigniVO kPSigniVO in
                        ...
                        select sk)
                    {
                        if (!kPSigniVOs.Exists((Map) => Map.Signi == kPSigniVO.Signi))
                        {
                ...
                if (include)
                {
                    ...
                    {
                        if (!kPSigniVOs.Exists((Map) => Map.Signi == signi.Signi))
                        {
        ...
        public List<KPDashaVO> Get_Antar_Dasha(DateTime dasha_starts, DateTime main_dasha_ends, short main_planet, List<KPPlanetMappingVO> kp_chart, bool include)
        {
            ...
            for (short i = 1; i <= 9; i = (short)(i + 1))
            {
                ...
                dashaStarts = dasha_starts.AddDays(num2);
                ...
                kPDashaVO.Duration = Get_Duration_String(Convert.ToInt64(endDate.Days));
                kPDashaVOs.Add(kPDashaVO);
                if (sno == 9)
                {
                    sno = 0;
        ...
        private string Get_Duration_String(long days)
        {
            ...
            long num1 = 0;
            long num2 = 0;
            num = Convert.ToInt64(Convert.ToInt64(Math.Floor(days / 30.41)) / 12);
            num1 = Convert.ToInt64(Convert.ToInt64(Math.Floor(days / 30.41)) - num * 12);
            num2 = Convert.ToInt64(days - num * 365);
            num2 = Convert.ToInt64(num2 - num1 * 30.41);
            if (num2 == 30 ? true : num2 == 31)
            {
                num1 = Convert.ToInt64(num1 + 1);
                num2 = 0;
            }
            if (num > 0)
            {
        ...
        public List<KPDashaVO> Get_Prayatntar_Dasha(List<KPDashaVO> antardasha, DateTime dasha_starts, DateTime main_dasha_ends, short main_planet, short antar_planet, List<KPPlanetMappingVO> kp_chart, bool include)
        {
            ...
            for (short i = 1; i <= 9; i = (short)(i + 1))
            {
                ...
                num3 = Convert.ToInt16(Convert.ToDouble(num3 * (num2 * 100 / 120) / 100));
                kPDashaVO = new KPDashaVO()
                {
                    Planet = (
                        from Map in kPMahadashaVOs
                ...
                TimeSpan endDate = kPDashaVO.EndDate - kPDashaVO.StartDate;
                kPDashaVO.Days = Convert.ToInt16(endDate.Days);
                kPDashaVO.Duration = Get_Duration_String(Convert.ToInt16(endDate.Days));
                kPDashaVOs.Add(kPDashaVO);
                if (sno == 9)
                {
                    sno = 0;
        ...
        public List<KPDashaVO> Get_Sukhsma_Dasha(List<KPDashaVO> antardasha, DateTime dasha_starts, DateTime main_dasha_ends, short main_planet, short antra_planet, short pryantar_planet, List<KPPlanetMappingVO> kp_chart, bool include)
        {
            ...
            kPMahadashaVOs = Fill_Mahadasha();
            KPDashaRashiVO kPDashaRashiVO = new KPDashaRashiVO();
            double num = Convert.ToInt16(kPMahadashaVOs.Where(Map => Map.Planet == antra_planet).SingleOrDefault().Years);
            DateTime dashaStarts = dasha_starts;
            short sno = kPMahadashaVOs.Where(Map => Map.Planet == pryantar_planet).SingleOrDefault().Sno;
            double num1 = Convert.ToDouble((from Map in kPMahadashaVOs where Map.Planet == antra_planet select Map).SingleOrDefault().Years);
            double num2 = 0;
            for (short i = 1; i <= 9; i = (short)(i + 1))
            {
                num2 = Convert.ToDouble(kPMahadashaVOs.Where(Map => Map.Sno == sno).SingleOrDefault().Years);
                double num3 = Convert.ToDouble(Math.Floor(Convert.ToDouble(num) * Convert.ToDouble(num1) / 10 * 30.41));
                num3 = Convert.ToInt16(Convert.ToDouble(num3 * (num1 * 100 / 120) / 100));
                num3 = Convert.ToInt16(Convert.ToDouble(num3 * (num2 * 100 / 120) / 100));

                kPDashaVO = new KPDashaVO();

                var planet = kPMahadashaVOs.Where(Map => Map.Sno == sno).SingleOrDefault().Planet;
                var nakLord = kp_chart.Where(Map => Map.Planet == planet).SingleOrDefault().Nak_Lord;

                kPDashaVO.Planet = planet;
                kPDashaVO.StartDate = dasha_starts;
                kPDashaVO.Signi = Get_Planet_Signis(kp_chart, planet, include);
                kPDashaVO.NakSigni = Get_Planet_Signis(kp_chart, nakLord, include);
                kPDashaVO.Signi_String = Get_Signi_String(planet, kp_chart, include);
                kPDashaVO.Nak_Signi_String = Get_Signi_String(nakLord, kp_chart, include);
                dashaStarts = dasha_starts.AddDays(num3);
                ...
                dashaStarts = dashaStarts.AddYears(kPMahadashaVOs.Where(Map => Map.Sno == sno).SingleOrDefault().Years);
            }
            return kPDashaVOs;
        }
            ...
            foreach (KPDashaVO kPDashaVO3 in kPDashaVOs1)
            {
                ...
                if (!both)
                {
                    ...
                    if (!fullmatch)
                    {
                        ...
                        num = 0;
                        while (num < strArrays1.Length)
                        {
                            if (!strArrays.Contains(strArrays1[num].Trim()))
                            {
                                num++;
                            }
                            else
                ...
                if (both)
                {
                    ...
                    if (!fullmatch)
                    {
                        ...
                        while (num < strArrays1.Length)
                        {
                            if (!strArrays3.Contains(strArrays1[num].Trim()))
                            {
                                num++;
                            }
                            else
        ...
        public bool isFewConditionMet(KPMixDashaVO mix, List<KPPlanetMappingVO> kp_chart, string houses)
        {
            ...
            else if (!(!isEmptyPass(mix.empty, kp_chart) || !isNotEmptyPass(mix.not_empty, kp_chart) || !isShubhPass(mix.shubh, kp_chart) ? false : isAshubhPass(mix.ashubh, kp_chart)))
            {
                flag = false;
            }
            else if (!(!isAndPlanetPass(mix.andplanet, kp_chart) || !isNotAndPlanetPass(mix.not_andplanet, kp_chart) || !isOrPlanetPass(mix.orplanet, kp_chart) ? false : isNotOrPlanetPass(mix.not_orplanet, kp_chart)))
            {
                flag = false;
            }
            else if (!isConjPass(mix.conj, mix.Planet1, kp_chart) ? false : isNotConjPass(mix.not_conj, mix.Planet1, kp_chart))
            {
                if (!isDrishtiPass(mix.drishti, mix.Planet1, kp_chart) ? true : !isNotDrishtiPass(mix.not_drishti, mix.Planet1, kp_chart))
                {
                    flag = false;
                    return flag;
                }
        ...
        public bool isYutiPass(KPMixDashaVO mix, List<KPPlanetMappingVO> kp_chart)
        {
            ...
            if (mix.yuticombi.Trim().Length <= 0 ? false : !(mix.yuticombi.Trim() == "0"))
            {
                ...
                for (int i = 0; i < strArrays2.Length; i++)
                {
                    ...
                    if (num2 == 1)
                    {
                        house = (
                            ...
                            select Map).SingleOrDefault().House;
                    }
                    if (num2 == 2)
                    {
                        num = (
                ...
                if (strArrays.Count() <= 0 ? false : mix.yutihouse != "0")
                {
                    if (!strArrays.Contains(house.ToString()))
                    {
                        flag = false;
                        return flag;
                    }
            ...
            if (shubh.Trim().Length <= 0 ? false : !(shubh.Trim() == "0"))
            {
                ...
                while (num < strArrays.Length)
                {
                    ...
                    if (!(
                        ...
                        select Map).ToList().SingleOrDefault().isbad)
                    {
                        num++;
                    }
                    else
            ...
            if (conj.Trim().Length <= 0 ? false : !(conj.Trim() == "0"))
            {
                ...
                for (int i = 0; i < strArrays.Length; i++)
                {
                    ...
                    short num = (
                        ...
                        where Map.Planet == planet
                        select Map).SingleOrDefault().House;
                    if (!isDrishti(Convert.ToInt16(str), planet, kp_chart) ? false : isDrishti(planet, Convert.ToInt16(str), kp_chart))
                    {
                        flag1 = false;
                    }
                    if (house == num)
        ...
        public bool isDrishti(short planet1, short planet2, List<KPPlanetMappingVO> kp_chart)
        {
            ...
            if ((
                ...
                select Map).SingleOrDefault().Bhav_Chalit_House != 8)
            {
                short bhavChalitHouse = (
                    from Map in kp_chart
                    where Map.Planet == 1
                    select Map).SingleOrDefault().Bhav_Chalit_House;
                short num = (
                    from Map in kp_chart
                    where Map.Planet == 2
                    select Map).SingleOrDefault().Bhav_Chalit_House;
                ...
                {
                    num5 = Convert.ToInt16(num4 - bhavChalitHouse5);
                    if (Math.Abs(num5) == 6)
                    {
                        flag = true;
                        return flag;
        ...
        public bool isDrishtiPass(string from_drishti, short to_drishti, List<KPPlanetMappingVO> kp_chart)
        {
            ...
            if (from_drishti.Trim().Length <= 0 ? false : !(from_drishti.Trim() == "0"))
            {
                ...
                while (num < strArrays.Length)
                {
                    if (!isDrishti(Convert.ToInt16(strArrays[num]), to_drishti, kp_chart))
                    {
                        num++;
                    }
                    else
